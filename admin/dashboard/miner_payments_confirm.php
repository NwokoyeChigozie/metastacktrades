...
$sql = "SELECT * FROM mining_payments WHERE id='$p_id'";
...
$reg_stamp = strtotime($rd_split[1] . '-' . $rd_split[0] . '-' . $rd_split[2] . ' ' . $reg_d[1]);
    ...
    $sql_t = "UPDATE `mining_payments` SET `status`='completed' WHERE `id`= '$p_id'";    
                                        if(mysqli_query($link, $sql_t)){
                                            $created_time = time();
                                            $created_at = date("d/m/Y", $created_time);
                    $w_date = date("d-m-Y", $created_time);
                    $sql= "INSERT INTO `history`(`u_id`,`username`, `type`,`amount`, `date`,`status`) VALUES ('$u_id','$username','BTC Mining','$history_amount','$w_date','completed')";
                    if(mysqli_query($link, $sql)){
                        echo "<div class='alert alert-success'>Payment successfully Completed!!</div>";
                        header("Refresh:2; url=miner_payments.php");   
